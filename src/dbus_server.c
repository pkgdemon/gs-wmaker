 ...
 *
 * 'org.wmaker.WMInterface' offers 3 methods:
 *
 ...
 *
 * $ gdbus introspect --session --dest org.wmaker.WMServer --object-path /org/example/WMObject
 */
DBusHandlerResult server_message_handler(DBusConnection *conn, DBusMessage *message, void *data)
{
	...
	} else if (dbus_message_is_method_call(message, "org.wmaker.WMInterface", "Echo")) {
		const char *msg;
	...
	} else if (dbus_message_is_method_call(message, "org.wmaker.WMInterface", "EmitSignal")) {

		if (!(reply = dbus_message_new_signal("/org/wmaker/WMObject",
						      "org.wmaker.WMInterface",
						      "OnEmitSignal")))
			goto fail;
...
static int dbus_run_loop(void)
{
	if (maincontext) {
		g_main_context_iteration(maincontext, FALSE);

		if (dbus_loop_timer) {
			WMDeleteTimerHandler(dbus_loop_timer);
			dbus_loop_timer = NULL;
		}
		dbus_loop_timer = WMAddTimerHandler(LOOP_INTERVAL, (WMCallback*)dbus_run_loop, NULL);
	}
}
...
int dbus_init()
{
  ...
  dbus_error_init(&err);
	...
	rv = dbus_bus_request_name(conn, "org.wmaker.WMServer", DBUS_NAME_FLAG_REPLACE_EXISTING , &err);
	if (rv != DBUS_REQUEST_NAME_REPLY_PRIMARY_OWNER) {
		fprintf(stderr, "Failed to request name on bus: %s\n", err.message);
		goto fail;
	}

	if (!dbus_connection_register_object_path(conn, "/org/wmaker/WMObject", &server_vtable, NULL)) {
		fprintf(stderr, "Failed to register a object path for 'WMObject'\n");
		goto fail;
	}
	...
	dbus_connection_setup_with_g_main(conn, maincontext);
